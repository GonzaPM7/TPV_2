		...
		if (gm->isRunning()) {
			for (int i = 0; i < asteroids_->size(); i++) {
				for (int j = 0; j < blackHoles_->size(); j++) {
					...
					if (asteroid->isActive() && blackhole->isActive() && Collisions::collidesWithRotation(asteroid, blackhole))
					{
						c->globalSend(this, msg::BlackholeAsteroidCollisionMsg(msg::None, msg::Broadcast, asteroid, blackhole));
					}
				}
			}
		}
...
void BlackHoleAsteroidCollision::receive(Container * c, const msg::Message & msg)
{
	switch (msg.type_)
	{
