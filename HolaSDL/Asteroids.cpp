...
void Asteroids::receive(const void *senderObj, const msg::Message &msg) {
  ...
  case msg::BULLET_ASTEROID_COLLISION:
    onCollisionWithBullet(msg);
    break;
  case msg::BLACKHOLE_ASTEROID_COLLISION:
    onCollisionWithBlackhole(msg);
    break;
  }
}

void Asteroids::onGameStart(const msg::Message &msg) { // enviar mensaje
  globalSend(this, msg::AsteroidsInfo(msg::Asteroids, msg::Broadcast,
                                      &getAllObjects()));
}

void Asteroids::onRoundStart() {
  setActive(true);
  for (int i = 0; i < 10; i++) {
    ...
    Vector2D v =
        (c - p).normalize() *
        (getGame()->getServiceLocator()->getRandomGenerator()->nextInt(1, 10) /
         20.0);
...
void Asteroids::onRoundOver() {
  deactiveAllObjects();
  setActive(false);
}

void Asteroids::onCollisionWithBullet(const msg::Message &msg) {
...
void Asteroids::onCollisionWithBlackhole(const msg::Message &msg) {
	Asteroid *asteroid = static_cast<const msg::BlackholeAsteroidCollisionMsg &>(msg)
